...
describe "Transactions Relationship Endpoints" do
  ...
  it "can return a transaction's invoice" do
   ...
   expect(json_response["data"]["attributes"]["id"]).to eq(@transaction_1.invoice.id)
   expect(json_response["data"]["attributes"]["customer_id"]).to eq(@transaction_1.invoice.customer.id)
   expect(json_response["data"]["attributes"]["merchant_id"]).to eq(@transaction_1.invoice.merchant.id)
   expect(json_response["data"]["attributes"]["status"]).to eq(@transaction_1.invoice.status)
  end
end
