...
describe "Customer Relationship Endpoints" do
  ...
  it "can return an customer's invoices" do
   ...
   expect(json_response["data"][0]["attributes"]["customer_id"]).to eq(@invoice_1.customer_id)
   expect(json_response["data"][0]["attributes"]["status"]).to eq(@invoice_1.status)
   expect(json_response["data"][1]["attributes"]["id"]).to eq(@invoice_5.id)
   expect(json_response["data"][1]["attributes"]["merchant_id"]).to eq(@invoice_5.merchant_id)
   expect(json_response["data"][1]["attributes"]["customer_id"]).to eq(@invoice_5.customer_id)
   expect(json_response["data"][1]["attributes"]["status"]).to eq(@invoice_5.status)
  end

  it "can return an customer's transactions " do
    ...
    expect(json_response["data"][0]["attributes"]["result"]).to eq(@transaction_1.result)
    expect(json_response["data"][1]["attributes"]["id"]).to eq(@transaction_5.id)
    expect(json_response["data"][1]["attributes"]["invoice_id"]).to eq(@transaction_5.invoice_id)
    expect(json_response["data"][1]["attributes"]["result"]).to eq(@transaction_5.result)
